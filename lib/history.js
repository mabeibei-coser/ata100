...
// 2026-06-04 起：薪酬域文档物理合并到 A800 doc-library（一库管两域，按 category 区分），
// 这里只读 A800 的 db，并在 SQL 里按 category='人才ATA' 过滤出薪酬域记录。
const DOC_DB_PATH =
...
/**
 * 查"我的"文档下载记录（只读 A800 doc-library document_downloads，JOIN documents 按 category 过滤薪酬域）。
 */
export function getDocDownloadHistory(phone, limit = 50) {
  ...
  try {
    ...
    const rows = db
      .prepare(
        `SELECT dd.id, dd.document_id, dd.document_title, dd.action, dd.created_at
         FROM document_downloads dd
         JOIN documents d ON d.id = dd.document_id
         WHERE dd.user_phone = ? AND d.category = '人才ATA'
         ORDER BY dd.created_at DESC LIMIT ?`
      )
      .all(phone, limit);
    return rows.map((r) => ({
      id: r.id,
    ...
    console.error("[history] 读 A800 文档下载记录失败:", err?.message || err);
    return [];
  } finally {
    if (db) db.close();
  }
